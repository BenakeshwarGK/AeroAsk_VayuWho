...
def get_llm_response(aircraft_id, user_query):
    ...
    # New descriptive prompt
    prompt = f"""You are an expert in military aircraft.

Your task is to generate a detailed and informative paragraph about the aircraft "{aircraft_id}".

Use the following context when relevant:

{context}

Make sure to include the aircraft’s manufacturer, origin country, speed, roles, and unique capabilities.
...
def predict():
    if 'image' not in request.files or 'question' not in request.form:
        return render_template('result.html', aircraft=None, answer="Invalid input.")

    image = request.files['image']
    question = request.form['question']

    # Predict the aircraft ID
    aircraft_id = predict_aircraft(image)
    
    # Get LLM-generated answer
    answer = get_llm_response(aircraft_id, question)

    return render_template('result.html', aircraft=aircraft_id, answer=answer)
